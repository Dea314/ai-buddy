...
export async function POST(
  ...
  try {
    ...
    const records = await memoryManager.readLatestHistory(buddyKey);

    if (records.length === 0) {
      await memoryManager.seedChatHistory(buddy.seed, "\n\n", buddyKey);
    }
    ...
    if (response !== undefined && response.length > 1) {
      memoryManager.writeToHistory("" + response.trim(), buddyKey);
